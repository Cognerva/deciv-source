...
class UnitUniquesTests {
    ...
    @Test
    fun `signed numeric unique parameters are portable across runtimes`() {
        val unit = game.createBaseUnit("Melee", "[+33]% Strength").apply {
            movement = 2
            strength = 10
        }

        Assert.assertEquals(listOf("33"), unit.uniqueObjects.single().params)
        Assert.assertTrue("Force evaluation should accept a leading plus parameter", unit.getForceEvaluation() > 0)
    }
