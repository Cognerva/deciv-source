...
//                (if (game.settings.showMinimap) minimapWrapper.height else 0f)
    ...
    override fun resize(width: Int, height: Int) {
        // Intentionally do not call super.resize here: SafeAreaExtendViewport refreshes from
        // render(), and the comparison must use the full resize dimensions rather than its inset
        // stage rectangle.
        resizeDeferTimer?.cancel()
        if (resizeDeferTimer == null && fullScreenWidth == width && fullScreenHeight == height) return
