...
abstract class BaseScreen : Screen {
    ...
    init {
        ...
        val viewport = SafeAreaExtendViewport(height, height) {
            game.settings.iosSafeAreaInsets && game.supportsSafeAreaInsets()
        }
        stage = UncivStage(viewport)
    ...
    /** The resize dimensions before the safe-area viewport removes platform insets. */
    val fullScreenWidth
        get() = (stage.viewport as? SafeAreaExtendViewport)?.lastFullWidth ?: stage.viewport.screenWidth

    /** The resize dimensions before the safe-area viewport removes platform insets. */
    val fullScreenHeight
        get() = (stage.viewport as? SafeAreaExtendViewport)?.lastFullHeight ?: stage.viewport.screenHeight
    ...
    override fun render(delta: Float) {
        val safeViewport = stage.viewport as? SafeAreaExtendViewport
        safeViewport?.refreshIfNeeded()

        // Clear the complete framebuffer first so the excluded safe-area region is a deliberate
        // black bar instead of a stale frame. Then clear only the stage rectangle with the app's
        // normal background colour.
        Gdx.gl.glClearColor(Color.BLACK.r, Color.BLACK.g, Color.BLACK.b, Color.BLACK.a)
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT)
        if (safeViewport?.hasSafeAreaInsets() == true) {
            Gdx.gl.glEnable(GL20.GL_SCISSOR_TEST)
            HdpiUtils.glScissor(
                safeViewport.screenX,
                safeViewport.screenY,
                safeViewport.screenWidth,
                safeViewport.screenHeight
            )
    ...
    override fun resize(width: Int, height: Int) {
        if (this !is RecreateOnResize) {
            ...
            if (fullScreenWidth == width && fullScreenHeight == height) return
