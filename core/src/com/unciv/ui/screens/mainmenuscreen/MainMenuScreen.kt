...
class MainMenuScreen: BaseScreen(), RecreateOnResize {
    ...
    companion object {
        ...
        /** Inner size of the Civilopedia and support buttons (effective size adds 2f for the thin circle) */
        const val buttonsSize = 60f
        /** Distance of the Civilopedia and support buttons from the stage edges */
    ...
    init {
        ...
        if (game.isStoreAvailable()) {
            val supporterButton = ImageGetter.getImage("OtherIcons/Heart").apply {
                color = if (game.settings.supporterUnlocked) Color.GOLD else Color.WHITE
            }
                .surroundWithCircle(buttonsSize, color = skinStrings.skinConfig.baseColor)
                .surroundWithThinCircle(Color.WHITE)
                .onActivation { SupportPopup(this@MainMenuScreen).open() }
            supporterButton.addTooltip("Support DeCiv", always = true)
            rightSideButtons.add(supporterButton)
        }

        val contactButton = ImageGetter.getImage("OtherIcons/Mail")
            .surroundWithCircle(buttonsSize, color = skinStrings.skinConfig.baseColor)
            .surroundWithThinCircle(Color.WHITE)
            .onActivation { SupportContact.open(this@MainMenuScreen) }
        contactButton.addTooltip("Contact support", always = true)
        rightSideButtons.add(contactButton)
        ...
        val versionLabel = "{Version} ${UncivGame.VERSION.toNiceString()}".toLabel()
        ...
        versionTable.touchable = Touchable.disabled
