...
class NewGameScreen(
    ...
    /** Local-only iOS harness entry point for the real Start Game action. */
    fun startGameForScreenshotHarness() = startGameAvoidANRs()
    ...
    private suspend fun checkConnectionToMultiplayerServer(): Boolean {
        return try {
            ...
            if (Multiplayer.usesDropbox()) {
                // Dropbox has no /isalive endpoint. Any HTTP response proves the
                // native transport can reach the service; HTTP errors are handled
                // later by the storage implementation.
                UncivKtor.client.get("https://content.dropboxapi.com")
                true
            } else {
                UncivGame.Current.onlineMultiplayer.multiplayerServer.checkServerStatus()
            }
        } catch(_: Throwable) {
            false
        }
    ...
    private suspend fun startNewGame() = coroutineScope {
        ...
        try {
            ...
            Log.error("Could not start new game", exception)
            launchOnGLThread {
                popup.apply {
