...
internal class AdvancedTab(
    ...
    override fun lateInitialize() {
        ...
        if (game.supportsSafeAreaInsets() || Display.hasCutout())
            addCutoutCheckbox()
    ...
    private fun addCutoutCheckbox() {
        val label = if (game.supportsSafeAreaInsets())
            "Keep interface clear of the notch"
        else
            "Enable using display cutout areas"
        val setting = if (game.supportsSafeAreaInsets()) settings::iosSafeAreaInsets else settings::androidCutout
        addCheckbox(label, setting) {
    ...
    private fun addFontSizeMultiplier() {
        addSlider("Font size multiplier", settings::fontSizeMultiplier, 0.3f, 1.5f, 0.05f) {
