...
class CrashScreen(val exception: Throwable) : BaseScreen() {
    ...
    /** Full report including save game/mods data, only ever used for the Copy button. */
    ...
    private fun makeTitleLabel() =
        "An unrecoverable error has occurred in DeCiv:".toLabel(fontSize = Constants.headingFontSize)
            .apply {
                wrap = true
    ...
    private fun makeInstructionLabel() =
        "{If this keeps happening, you can try disabling mods.}".toLabel()
            .apply {
                wrap = true
                setAlignment(Align.center)
            }
    ...
    private fun makeActionButtonsTable(): Table {
        ...
        val closeButton = IconTextButton("Close DeCiv", fontSize = Constants.headingFontSize)
            .onClick { Gdx.app.exit() }
        val emailButton = IconTextButton("Email report", fontSize = Constants.headingFontSize)
            .onClick { SupportContact.open(this@CrashScreen, displayedText) }

        val buttonsTable = Table()
        buttonsTable.add(copyButton)
            .pad(10f)
        buttonsTable.add(emailButton)
            .pad(10f)
