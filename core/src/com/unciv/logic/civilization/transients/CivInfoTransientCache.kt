...
class CivInfoTransientCache(val civInfo: Civilization) {
    ...
    fun setTransients() {
        ...
        fun INonPerpetualConstruction.requiredResourcesForStartup(): Set<String> = try {
            requiredResources(state)
        } catch (exception: RuntimeException) {
            // A single malformed/runtime-null construction must not abort GameStarter's
            // transient initialization for every civilization in the new game.
            Log.error("Could not calculate resource requirements for $name during game startup", exception)
            emptySet()
        }
        val buildingsToRequiredResources = ruleset.buildings.values
                .filter { civInfo.getEquivalentBuilding(it) == it }
                .associateWith { it.requiredResourcesForStartup() }
