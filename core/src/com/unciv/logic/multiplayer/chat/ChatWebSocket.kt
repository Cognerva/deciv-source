...
object ChatWebSocket {
    ...
    private var lastRetry = Instant.now()
    ...
    private val client = UncivKtor.createClient {
        install(WebSockets) {
    ...
    private fun handleWebSocketThrowables(t: Throwable) {
        ...
        if (reconnectionAttempts == 0) {
            ...
            val now = Instant.now()
            val elapsed = (now.toEpochMilli() - lastRetry.toEpochMilli()).milliseconds
            print(" (Last retry was ${elapsed.toString(DurationUnit.SECONDS, 2)} ago)")
            lastRetry = now
        }
