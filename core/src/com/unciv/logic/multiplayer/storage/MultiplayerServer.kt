...
private const val SERVER_STATUS_TIMEOUT_MILLIS = 15_000L
...
class MultiplayerServer(
    ...
    suspend fun checkServerStatus(): Boolean {
        return withTimeoutOrNull(SERVER_STATUS_TIMEOUT_MILLIS) {
            ...
            if (resp.status.isSuccess()) {
                ...
                setFeatureSet(
                    try {
                        json().fromJson(ServerFeatureSet::class.java, responseBody)
                    } catch (_: Exception) {
            ...
            // Keep a broken or unreachable server from leaving a UI popup open forever.
            false
        }
    }
