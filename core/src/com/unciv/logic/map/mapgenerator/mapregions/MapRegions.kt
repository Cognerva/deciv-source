...
fun Tile.getTileFertility(checkCoasts: Boolean): Int {
    ...
    for (terrain in allTerrains) {
        if (terrain.hasUnique(UniqueType.OverrideFertility))
            return terrain.getMatchingUniques(UniqueType.OverrideFertility).first().params[0].toMapGenerationInt()
        else
            fertility += terrain.getMatchingUniques(UniqueType.AddFertility)
                .sumOf { it.params[0].toMapGenerationInt() }
    }
 ...
 * regular JVM. Map-generation uniques use signed values such as "[+4]".
 */
private fun String.toMapGenerationInt(): Int = removePrefix("+").toInt()
