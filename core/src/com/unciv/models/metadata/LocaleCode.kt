...
enum class LocaleCode(
    ...
    /**
     * RoboVM's Java runtime predates Locale.forLanguageTag. Keep the language
     * and country portions, which are the parts Unciv uses for collation and
     * number formatting. Unicode extension subtags are intentionally ignored
     * on that runtime and remain documented by [languageTag].
     */
    @Readonly fun locale(): Locale {
        val parts = languageTag.split('-')
        val language = parts.firstOrNull().orEmpty()
        val country = parts.drop(1).firstOrNull { it.length == 2 || it.length == 3 }.orEmpty()
