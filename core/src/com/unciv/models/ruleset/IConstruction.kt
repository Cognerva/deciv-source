...
interface INonPerpetualConstruction : IConstruction, INamed, IHasUniques {
    ...
    override fun requiredResources(state: GameContext): Set<String> {
        ...
        // Keep this calculation deliberately imperative. On RoboVM, a malformed or
        // partially initialized ruleset object can fail while evaluating one of these
        // queries. That object should not prevent a new game from starting.
        try {
            resources.addAll(getResourceRequirementsPerTurn(state).keys)
        } catch (exception: RuntimeException) {
            Log.error("Could not read per-turn resource requirements for $name", exception)
        }

        try {
            for (unique in getMatchingUniques(UniqueType.CostsResources, state)) {
                val resource = unique.params.getOrNull(1)
                if (resource != null) resources.add(resource)
                else Log.error("Ignoring malformed stockpiled resource requirement on $name: ${unique.text}")
            }
        } catch (exception: RuntimeException) {
            Log.error("Could not read stockpiled resource requirements for $name", exception)
        }
        return resources
    }
