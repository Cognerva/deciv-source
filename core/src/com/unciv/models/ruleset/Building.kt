...
class Building : RulesetStatsObject(), INonPerpetualConstruction {
    ...
    override fun getResourceRequirementsPerTurn(state: GameContext?): Counter<String> {
        ...
        // RoboVM can return a null value for the shared Counter.ZERO singleton here while
        // initializing a new game. An ordinary empty counter has the same semantics and keeps
        // this resource query safe on iOS.
        if (uniques.none() && requiredResource == null) return Counter()
