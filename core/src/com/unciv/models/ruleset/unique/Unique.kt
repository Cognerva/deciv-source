...
/**
 * RoboVM's Integer.parseInt rejects the optional leading '+' accepted by the
 * regular JVM. Numeric unique parameters are commonly written as `[+33]`.
 * Keep the original unique text for display, but canonicalize numeric params
 * once so every ruleset consumer gets a portable value.
 */
private val positiveNumericParameter = Regex("""^\+[0-9]+(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?$""")

private fun String.normalizeLeadingPlusForRoboVM() =
    if (positiveNumericParameter.matches(this)) drop(1) else this
...
class Unique(val text: String, val sourceObjectType: UniqueTarget? = null, val sourceObjectName: String? = null) {
    ...
    private val rawParams = text.getPlaceholderParameters()
    val params = rawParams.map { it.normalizeLeadingPlusForRoboVM() }
    ...
    fun getReplacementText(ruleset: Ruleset): String {
        ...
        for (possibleUnique in possibleUniques) {
            ...
            for (parameter in possibleUnique.replace('<', ' ').getPlaceholderParameters()) {
                ...
                var replacementText = rawParams[parameterNumberInDeprecatedUnique]
